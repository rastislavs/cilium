...
// PolicyStatementName returns a unique route policy statement name for the provided peer and advertisement type.
// If there is a need for multiple route policies per advertisement type, unique resourceID can be provided.
func PolicyStatementName(peer string, advertType v2.BGPAdvertisementType, resourceID string) string {
	if resourceID == "" {
		return fmt.Sprintf("%s-%s", peer, advertType)
	}
	return fmt.Sprintf("%s-%s-%s", peer, advertType, resourceID)
}
...
func reconcileDesiredRoutePolicyStatements(tx statedb.WriteTxn, table statedb.RWTable[*bgpTables.DesiredRoutePolicy],
	...
	desiredByName := make(map[string]*bgpTables.DesiredRoutePolicy, len(desiredStatements))
	for _, statement := range desiredStatements {
		desiredByName[statement.StatementName()] = statement
	}

	existingByName := make(map[string]*bgpTables.DesiredRoutePolicy)
	for existing := range table.List(tx, bgpTables.DesiredRoutePoliciesByInstanceOwnerResource(instance, owner, resource)) {
		existingByName[existing.StatementName()] = existing
	}

	// delete stale statements
	for name, existing := range existingByName {
		if _, isDesired := desiredByName[name]; !isDesired {
			if _, _, err := table.Delete(tx, existing); err != nil {
				return fmt.Errorf("error deleting desired route policy statement %s: %w", name, err)
			}
		}
	}
	...
	for name, desired := range desiredByName {
		...
		if _, _, err := table.Insert(tx, desired); err != nil {
			return fmt.Errorf("error inserting desired route policy statement %s: %w", name, err)
		}
	}
