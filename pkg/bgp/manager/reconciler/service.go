...
func (r *ServiceReconciler) Cleanup(i *instance.BGPInstance) {
	if i != nil {
		if err := cleanupDesiredRoutePolicyStatements(r.db, r.desiredRoutePolicyTable, i.Name, r.Name()); err != nil && r.logger != nil {
...
func (r *ServiceReconciler) getDesiredSvcRoutePolicyStatements(p ReconcileParams, resourceKey resource.Key, desiredPeerAdverts PeerAdvertisements, svc *loadbalancer.Service, rx statedb.ReadTxn) ([]*bgpTables.DesiredRoutePolicy, error) {
	desiredSvcPolicies := make(map[string]*bgpTables.DesiredRoutePolicy)
			...
			for _, advert := range adverts {
				...
				for advertType, prefixes := range typedPrefixes {
					...
					for _, statement := range statements {
						existingPolicy := desiredSvcPolicies[statement.Name]
						if existingPolicy != nil {
							// Statement with the same name already exists. This happens if the same service
							...
							desiredSvcPolicies[statement.Name] = &bgpTables.DesiredRoutePolicy{
...
func (r *ServiceReconciler) getServiceRoutePolicyStatements(peer PeerID, family types.Family, svc *loadbalancer.Service, svcPrefixes []netip.Prefix, advert v2.BGPAdvertisement, advertType v2.BGPServiceAddressType) ([]*types.RoutePolicyStatement, error) {
	...
	name := PolicyStatementName(peer.Name, advert.AdvertisementType, fmt.Sprintf("%s-%s-%s", svc.Name.Name(), svc.Name.Namespace(), advertType))
