...
func Test_AdvertisedPathAttributes(t *testing.T) {
	...
	var steps = []struct {
		...
		{
			description: "advertise service IP with extended community",
			op:          "update",
			advertiseAttributes: []v2alpha1.CiliumBGPPathAttributes{
				{
					SelectorType: v2alpha1.CiliumLoadBalancerIPPoolSelectorName,
					Communities: &v2alpha1.BGPCommunities{
						Extended: []v2alpha1.BGPExtendedCommunity{
							{
								SubType: "RouteTarget",
			...
			expectedRouteEvents: []routeEvent{
				{
					...
					extraPathAttributes: []bgp.PathAttributeInterface{
						...
						bgp.NewPathAttributeExtendedCommunities([]bgp.ExtendedCommunityInterface{
							bgp.NewTwoOctetAsSpecificExtended(bgp.EC_SUBTYPE_ROUTE_TARGET, 65100, 4294967295, true),
						}),
					},
				},
			},
	...
	for _, step := range steps {
		t.Run(step.description, func(t *testing.T) {
			...
			receivedRouteMatch := func() bool {
				...
				receivedEvents, err := gobgpPeers[0].getRouteEvents(testCtx, len(step.expectedRouteEvents))
				require.NoError(t, err, step.description)
				equal := reflect.DeepEqual(step.expectedRouteEvents, receivedEvents)
				if !equal {
					t.Logf("route events not (yet) equal - expected: %v, actual: %v", step.expectedRouteEvents, receivedEvents)
				}
				return equal
			}
