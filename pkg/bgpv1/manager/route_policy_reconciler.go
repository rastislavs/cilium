...
func (r *RoutePolicyReconciler) pathAttributesToPolicy(attrs v2alpha1api.CiliumBGPPathAttributes, neighborAddress string, params ReconcileParams) (*types.RoutePolicy, error) {
	...
	if attrs.Communities != nil {
		...
		sort.Slice(extCommunities, func(i, j int) bool {
			return extCommunities[i].SubType < extCommunities[j].SubType || extCommunities[i].Value < extCommunities[j].Value
		})
	}
	...
	if len(v4Prefixes) > 0 {
		policy.Statements = append(policy.Statements, policyStatement(neighborAddress, v4Prefixes,
			attrs.LocalPreference, communities, largeCommunities, extCommunities))
	}
	if len(v6Prefixes) > 0 {
		policy.Statements = append(policy.Statements, policyStatement(neighborAddress, v6Prefixes,
			attrs.LocalPreference, communities, largeCommunities, extCommunities))
	}
	return policy, nil
}
...
func policyStatement(neighborAddr string, prefixes []*types.RoutePolicyPrefixMatch, localPref *int64,
	communities, largeCommunities []string, extCommunities []types.ExtendedCommunity) *types.RoutePolicyStatement {
	return &types.RoutePolicyStatement{
		...
		Actions: types.RoutePolicyActions{
			...
			AddExtendedCommunities: extCommunities,
		},
	}
}
