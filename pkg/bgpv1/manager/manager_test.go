...
func TestGetRoutes(t *testing.T) {
	...
	for _, tt := range table {
		t.Run(tt.name, func(t *testing.T) {
			...
			// ensure retrieved routes count matches expected prefixes count
			require.Equal(t, len(routes), len(tt.expectedPrefixes), "retrieved routes count does not match expected prefix count")

			// ensure we see expected prefixes in retrieved routes
			for _, cidr := range tt.expectedPrefixes {
				var seen bool
				for _, route := range routes {
					if route.Prefix == cidr.String() {
						seen = true
					}
				}
				require.True(t, seen, "expected CIDR %v not found in retrieved routes", cidr)
			}
		})
	}
}
