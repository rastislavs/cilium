...
var BgpRoutesCmd = &cobra.Command{
	Use:   "routes <available | advertised> <afi> <safi> [vrouter <asn>] [neighbor <address>]",
	Short: "List routes in the BGP CPlane's RIBs",
  ...
  Get IPv4 unicast routes advertised to specific peer:
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if params.TableType == adjRIBOutTableType {
			addr, err := parseBGPNeighborAddr(args)
			if err != nil {
				Fatalf("failed to parse neighbor address: %s\n", err)
			}
...
func parseBGPNeighborAddr(args []string) (string, error) {
	if len(args) == 0 || args[0] != neighborKW {
		return "", fmt.Errorf("missing `neighbor` parameter")
	}

	if len(args) < 2 {
		return "", fmt.Errorf("missing neighbor IP address")
	}
	addr, err := netip.ParseAddr(args[1])
	if err != nil {
		return "", fmt.Errorf("invalid neighbor IP address: %w", err)
	}
